...
class LeverSource(JobSource):
    def fetch_jobs(self, company: CompanyConfig) -> list[RawJob]:
        ...
        return [RawJob(source_id=item["id"], title=item["text"], official_url=item["hostedUrl"], location=item.get("categories", {}).get("location"), description=item.get("descriptionPlain"), team=item.get("categories", {}).get("team"), employment_type=item.get("categories", {}).get("commitment")) for item in response.json()]
